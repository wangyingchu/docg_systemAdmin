...
public class AllowRepeatAttributeEditorView extends VerticalLayout {
    ...
    public AllowRepeatAttributeEditorView(RelationAttachKind relationAttachKind) {
        ...
        select = new Select<>();
        select.setItems("不允许重复创建相同类型的关系","允许重复创建相同类型的关系");
        select.setWidth(220, Unit.PIXELS);
        select.setEmptySelectionAllowed(false);
        if(relationAttachKind.isRepeatableRelationKindAllow()){
            select.setValue("允许重复创建相同类型的关系");
        }else{
            select.setValue("不允许重复创建相同类型的关系");
        }
    ...
    private void updateAttributeValue() {
        boolean isRepeatableRelationKindAllow = relationAttachKind.isRepeatableRelationKindAllow();
        String selectedValue = this.select.getValue();
        if(selectedValue.equals("允许重复创建相同类型的关系")){
            isRepeatableRelationKindAllow = true;
        }else{
            isRepeatableRelationKindAllow = false;
        }
        relationAttachKind.setAllowRepeatableRelationKind(isRepeatableRelationKindAllow);
        CommonUIOperationUtil.showPopupNotification("isRepeatableRelationKindAllow 状态更新成功", NotificationVariant.LUMO_SUCCESS);

        if(this.parentRelationAttachKindManagementUI != null){
            this.parentRelationAttachKindManagementUI.updateAllowRepeatAttribute(relationAttachKind);
        }
